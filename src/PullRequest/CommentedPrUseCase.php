...
final readonly class CommentedPrUseCase implements PrEventHandlerInterface
{
    public function __construct(
        ...
        private string $commentedPrReaction = 'speech_balloon',
    ) {
    }
    ...
    public function handle(WebHookTransfer $webHookTransfer): void
    {
        ...
        // Approve
        $this->slackApiClient->addReaction($slackMapping, (string) $slackMessage->getTs(), $this->commentedPrReaction);
    }
    ...
    public function isAlreadyCommented(
        ...
        return array_any(
            ...
            fn($reactionItem) => $reactionItem['name'] === $this->commentedPrReaction && in_array(
                $botUserId,
                $reactionItem['users'],
                true
            )
